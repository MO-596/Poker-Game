...
void Game<U>::evaluteHand(const vector<DeckOfCards<U>>& cards, int& outRankValue, int& outSuitValue,string& outHandName)
{
//  int rankCount[13] = {0};
//  int suitCount[4] = {0};
  ...
  vector<int> tripRank;
  vector<int> pairRank;
  detectMultiples(fourKindRank, tripRank, pairRank);
...
*/
   ...
   else if (!tripRank.empty())
   {
        // Three of a Kind only
       outRankValue = 3;
       outHighCard  = tripRank.back;
       outHandName  = "Three of a Kind";
   }
   else if (pairRank.size() >= 2)
   {/*
      ...
      outHighCard  = pairRank.back(); // highest pair
      outHandName  = "Two Pair";
   }
   else if (pairRank.size() == 1)
   {
...
//////////////////////////////////////////////////////
template <typename U>// Helper Function
void Game<U>::detectMultiples(int& fourKindRank, std::vector<int>& tripRank, std::vector<int>& pairRanks)
{
...
*/
 ...
 std::sort(tripRank.begin(),tripRank.end());
	...
	int& flushSuit, int& flushHighRank, bool& hasStraightFlush, int& hasStraightFlushRank)
{
  ...
  for(int r : ranksBySuit[flushSuit])
  {
    if(r >= 0 && r < 13)
    {
      presentSF[r] = true;
